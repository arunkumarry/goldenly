...
export default function Home() {
  ...
  const sendMessage = async (messageToSend = message) => {
    ...
    try {
      const result = await askCareAgent(prompt);
  ...
  const renderCare = () => <>
    ...
    {careSection === "Appointments" && <View style={styles.infoCard}><Text style={styles.cardTitle}>Physiotherapy visit</Text><Text style={styles.detail}>Today · 10:00 AM · Ravi Kumar</Text><Text style={styles.safeNote}>Demo provider contact for the hackathon.</Text><ActionButton secondary onPress={() => callPhone("+91 98765 43212")}>Call provider</ActionButton></View>}
