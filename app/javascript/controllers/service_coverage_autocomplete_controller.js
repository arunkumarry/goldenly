...
export default class extends Controller {
  static targets = ["input", "results", "placeId", "address", "city", "region", "country", "countryCode", "latitude", "longitude"]
  ...
  async fetchSuggestions(input) {
    try {
      ...
      if (!response.ok) throw new Error(payload.error || "Coverage suggestions are unavailable.")
  ...
  async select(event) {
    ...
    try {
      ...
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error || "Coverage details are unavailable.")
  ...
  renderSuggestions(suggestions) {
    ...
    suggestions.forEach((suggestion) => {
      ...
      button.dataset.action = "service-coverage-autocomplete#select"
  ...
  assign(place) {
    this.inputTarget.value = [place.city, place.region, place.country].filter(Boolean).join(", ") || place.address || ""
    this.set("placeId", place.place_id)
    this.set("address", place.address)
  ...
  clearSelectedPlace() {
    ["placeId", "address", "city", "region", "country", "countryCode", "latitude", "longitude"].forEach((target) => this.set(target, ""))
  }
