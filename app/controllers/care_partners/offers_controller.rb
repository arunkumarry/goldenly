class CarePartners::OffersController < CarePartners::BaseController
  def index
    expire_stale_offers
    @offers = current_care_partner_account.service_offers.includes(service_request: :service_catalog).order(expires_at: :asc)
  end

  def accept
    offer = current_care_partner_account.service_offers.find(params[:id])
  ...
  def expire_stale_offers
    current_care_partner_account.service_offers.offered.where("expires_at <= ?", Time.current).update_all(status: "expired", responded_at: Time.current, updated_at: Time.current)
  end
end
