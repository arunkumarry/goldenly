class Moderation::EarningsController < Moderation::BaseController
  ...
  def index
    @entries = EarningsLedgerEntry.includes(:care_partner_account, service_assignment: { service_request: :service_catalog }).order(created_at: :desc)
  end
  ...
  def update
    ...
    ModeratorReview.create!(care_partner_account: entry.care_partner_account, reviewer: current_user, decision: :payout_released, reason: "Manual payout marked #{status}.")
